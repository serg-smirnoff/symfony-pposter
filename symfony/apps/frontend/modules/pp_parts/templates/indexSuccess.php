...
<div id="itemsInner">
	...
	<div class="clear"></div>
	<div id="text0">
		<p>Вы можете подобрать и <strong>купить любой фото постер</strong> из нашего каталога. Постеры в каталоге сформированы в несколько тематических рубрик.</p>
	</div>
</div>
</div>
<div id="catalog" style="position: relative; top: -50px;">
	<div id="catalogInner">
	<?php $i = 1;?>
	<?php if (sizeOf ($parts) > 0) {?><div class="parts"><?php foreach ($parts as $item) {?><div <?php if (($i!=3) && ($i!=6)) echo "class=\"part\""; ?>><a href="http://p-poster.com/parts/id/<?php echo $item->getId(); ?>/" title="<?php echo $item->getName(); ?>"><img src="/uploads/photos/<?php echo $item->getPhoto(); ?>" alt="<?php echo $item->getName(); ?>" /></a>
	<a href="http://p-poster.com/parts/id/<?php echo $item->getId(); ?>/" title="<?php echo $item->getName(); ?>"><span class="descr">
	<?php
	 $cc = new Criteria();
...
<div><a href="http://p-poster.com/photos/cart/id/<?php echo $item->getId(); ?>/" title="Добавить в корзину"><img src="http://p-poster.com/images/b-addtocard.jpg" alt="Добавить в корзину" /></a></div>
</div>
<div class="clear"></div>
<?php }?>
</div>
