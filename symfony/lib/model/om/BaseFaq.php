...
/**
 * Base class that represents a row from the 'pp_faq' table.
 *
 * 
 *
...
abstract class BaseFaq extends BaseObject  implements Persistent {
  ...
  const PEER = 'FaqPeer';
	...
	/**
	 * Initializes internal state of BaseFaq object.
	 * @see        applyDefaults()
	 */
	...
	public function setMetaId($v)
	{
		...
		if ($this->meta_id !== $v) {
			$this->meta_id = $v;
			$this->modifiedColumns[] = FaqPeer::META_ID;
		}
	...
	public function setOrd($v)
	{
		...
		if ($this->ord !== $v) {
			$this->ord = $v;
			$this->modifiedColumns[] = FaqPeer::ORD;
		}
	...
	public function setIsActive($v)
	{
		...
		if ($this->is_active !== $v || $v === false) {
			$this->is_active = $v;
			$this->modifiedColumns[] = FaqPeer::IS_ACTIVE;
		}
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(FaqPeer::IS_ACTIVE))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->ord = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			...
			return $startcol + 7; // 7 = FaqPeer::NUM_COLUMNS - FaqPeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseFaq:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseFaq:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseFaq:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(FaqPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseFaq:save:post') as $callable)
    {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FaqPeer::META_ID)) $criteria->add(FaqPeer::META_ID, $this->meta_id);
		...
		if ($this->isColumnModified(FaqPeer::ORD)) $criteria->add(FaqPeer::ORD, $this->ord);
		if ($this->isColumnModified(FaqPeer::IS_ACTIVE)) $criteria->add(FaqPeer::IS_ACTIVE, $this->is_active);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseFaq:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseFaq::%s', $method));
    }
